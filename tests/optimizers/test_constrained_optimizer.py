...
class TestConstrainedOptimizer(ABCTestConstrainedOptimizer):
    ...
    def test_rosenbrock(self, options):
        ...
        expected_cost = 0.
        assert np.isclose(expected_cost, 0., rtol=1e-03)
    ...
    def test_rosenbrock_disk(self, options):
        ...
        opt = ConstrainedOptimizerPSO(10, 2, options=options)
        cost, pos = opt.optimize(rosenbrock, n_disk_constraint, 5000, r=2.)
        expected_pos = np.array([1., 1.])
        expected_cost = 0.
    ...
    def test_himmelblau(self, options):
        ...
        expected_pos = [np.array([-2.805118, 3.131312]),
    ...
    def test_himmelblau_disk(self, options, init_pos):
        ...
        x_min = -1 * x_max
        bounds = (x_min, x_max)
        opt = ConstrainedOptimizerPSO(10, 2, bounds=bounds, options=options, init_pos=init_pos)
        cost, pos = opt.optimize(himmelblau, n_disk_constraint, 10000, r=3.)
    ...
    def test_himmelblau_constrained(self, options, sum_under, init_pos):
        ...
        bounds = (x_min, x_max)
        opt = ConstrainedOptimizerPSO(10, 2, bounds=bounds, options=options, init_pos=init_pos)
        cost, pos = opt.optimize(himmelblau, sum_under, 5000, s=2.)
        expected_pos_1 = np.array([-2.805118, 3.131312])
        expected_pos_2 = np.array([3.584428, -1.848126])
        expected_cost = 0.
        assert np.isclose(expected_cost, 0., rtol=1e-03)
        assert pos == pytest.approx(expected_pos_1, rel=1e-03) or pos == pytest.approx(expected_pos_2, rel=1e-03)
